...
유저가 입력한 꽃/그린 종류, 증정 상황, 수령인 정보, 요청된 문체(격식/비격식)를 종합하여 마치 한 편의 짧은 시나 수필 같은 네이밍과 이야기를 지어줘.

[작성 제약사항 및 규칙]
1. 네이밍(name): 시적이고 함축적인 **4~7자**의 짧은 이름으로 지어줄 것.
2. 이야기(story): 꽃말과 문학적 심상을 자연스럽게 엮어낸 감동적인 이야기로 **정확히 4~5문장** 작성할 것.
...
6. 응답 형식: 반드시 JSON 형식으로 {"name": "이름", "story": "이야기내용"}만 반환할 것.`,
    });

...
import { CheckCircle, Clock, User, Phone, Sparkles } from "lucide-react";
import Link from "next/link";
import OrderNamingPanel, { type SavedNaming } from "./OrderNamingPanel";
...
export default function MatchedClient({ acceptedQuotes }: { acceptedQuotes: any[] }) {
  ...
  const handlePanelClose = (quoteId: string, saved?: SavedNaming) => {
    if (saved) {
      setSavedNamings((prev) => ({ ...prev, [quoteId]: saved }));
    }
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      {acceptedQuotes.map((quote) => {
        ...
        const saved = savedNamings[quote.id];
          ...
          <div key={quote.id} className="flex flex-col">
            ...
            {isActive && (
              <OrderNamingPanel
                ...
                initialData={saved}
                onClose={(savedData) => handlePanelClose(quote.id, savedData)}
              />
            )}
          </div>
        );
